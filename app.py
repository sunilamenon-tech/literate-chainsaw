...
# DISPLAY CHAT + CHEAT SHEET BUTTON LOGIC
for i, message in enumerate(st.session_state.messages):
    with st.chat_message(message["role"]):
        st.markdown(message["content"])
        
        # Only show button if it's the AI's last message and NOT already a cheat sheet
...
if st.session_state.messages[-1]["role"] == "user":
    with st.chat_message("assistant"):
        with st.spinner('FocusFlow is thinking...'):
            ...
            # Logic: If they click the button or type it, skip Socratic questions
            if "cheat sheet" in st.session_state.messages[-1]["content"].lower():
                full_prompt = f"Context: {exam_goal}, {current_topic}. Provide a high-yield cheat sheet for: {st.session_state.messages[-2]['content']}"
            else:
