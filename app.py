...
with st.sidebar:
    ...
    if st.button("Sync My Goal"):
        st.session_state.messages = [{"role": "assistant", "content": f"Context updated! Prepping for {exam_goal}. Let's master {current_topic}. What's the biggest challenge?"}]
        st.rerun()
...
# FEATURE: The "Emergency" Cheat Sheet Button
...
if prompt := st.chat_input("What's on your mind?"):
    ...
    with st.chat_message("assistant"):
        with st.spinner('FocusFlow is thinking...'):
            ...
            # The logic: If user asks for cheat sheet, skip the questions!
            if "cheat sheet" in prompt.lower() or "give me the answer" in prompt.lower():
                full_prompt = f"Context: {context}. Provide a high-yield cheat sheet for: {current_topic}."
            else:
                full_prompt = f"Context: {context}. User asks: {prompt}. Ask one Socratic diagnostic question first."
