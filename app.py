...
# ============================================================
# ERROR BOUNDARY — Never let the app crash to blank screen
# ============================================================
try:
    # ============================================================
    # PAGE CONFIG
    ...
    # ============================================================
    # CHECK SECRETS FIRST — Show setup screen if missing
    # ============================================================
    ...
    if not has_openrouter and not has_google:
        ...
        <div class='setup-box'>
            ...
            <p>You need to add an API key to use FocusFlow. Choose one option:</p>
        </div>
        """, unsafe_allow_html=True)
        
        st.markdown("### Option 1: OpenRouter (Recommended — Free, Easy)")
        st.markdown("""
        <div class='setup-step'>
            <b>Step 1:</b> Go to <a href='https://openrouter.ai/keys' target='_blank'>openrouter.ai/keys</a><br>
            <b>Step 2:</b> Sign up free → Create API key<br>
            <b>Step 3:</b> In your Streamlit app settings, add:<br>
            <code>OPENROUTER_API_KEY = "your-key-here"</code>
        </div>
        """, unsafe_allow_html=True)
        ...
        <div class='setup-step'>
            ...
            <b>Step 3:</b> In your Streamlit app settings, add:<br>
            <code>GOOGLE_API_KEY = "your-key-here"</code>
        </div>
        """, unsafe_allow_html=True)
        
        st.error("⛔ Add either OPENROUTER_API_KEY or GOOGLE_API_KEY to your Streamlit secrets to continue.")
        st.stop()
...
"""
            ...
            # ==================== GOOGLE GEMINI ====================
                ...
                if 'candidates' in response and response['candidates']:
                    ...
                    return f"⚠️ Google AI Error: {error}", "error"
    ...
    if not current_messages:
        ...
        provider_label = "OpenRouter" if st.session_state.ai_provider == "openrouter" else "Google AI"
...
Keep crushing it! 🚀"""
    ...
    # This catches ANY error and shows it instead of blank screen
    st.error(f"🚨 APP CRASHED: {str(e)}")
    st.code(f"Error type: {type(e).__name__}\n\nFull traceback:\n{str(e)}")
    st.info("💡 Check your Streamlit logs (Manage app → Logs) for the full error traceback.")
