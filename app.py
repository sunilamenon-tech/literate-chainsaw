...
with st.sidebar:
    ...
    if st.button("Sync My Goal"):
        st.session_state.messages = [{"role": "assistant", "content": f"Context updated! I see you're aiming for {exam_goal} and have a test on {test_date}. Let's master {current_topic}. What's the biggest challenge you're facing in this topic right now?"}]
        st.rerun()
...
if prompt := st.chat_input("What's on your mind?"):
    ...
    with st.chat_message("assistant"):
        with st.spinner('FocusFlow is thinking...'):
            ...
            full_prompt = f"You are a coach. Context: {context}. User asks: {prompt}. Provide mnemonics, cheat sheets, or a mini-study plan."
