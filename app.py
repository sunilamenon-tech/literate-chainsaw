...
for i, message in enumerate(st.session_state.messages):
    with st.chat_message(message["role"]):
        ...
        # LOGIC: Only show button if:
        # 1. It is AI.
        # 2. i > 3 (Conversation has some depth).
        # 3. AI asked a question (?) and NOT already a cheat sheet.
        if message["role"] == "assistant" and i > 3: 
    ...
    with st.chat_message("assistant"):
        with st.spinner('Thinking...'):
            try:
                ...
                elif any(x in user_msg for x in ["hi", "hello", "hey"]):
                    full_prompt = f"The user said '{user_msg}'. Respond naturally as a mentor. Do not ask a quiz question."
                else:
                    full_prompt = f"Context: {current_topic}. User: {user_msg}. Rules: Act as Socratic Coach. Do not agree immediately. Ask ONE diagnostic question."
                ...
                st.error("AI is catching its breath. Try again!")
